...
export interface SignUpIndividualPayload {
    email: string
    username: string
    password: string
}
...
export function useSignUpIndividual() {
    ...
    const signUp = useCallback(async (payload: SignUpIndividualPayload): Promise<SignUpResponse | null> => {
        setLoading(true)
        setError(null)
        try {
            const res = await axios.post(`${baseURL}/api/v1/auth/sign_up/individual`, payload)
            const data = res.data || {}
...
export default useSignUpIndividual
